...
import {
  ...
  Rocket, Database, KeyRound, Package, Terminal,
    ...
    <header className="sticky top-0 z-50 w-full border-b border-border/60 bg-background/80 backdrop-blur-md">
      <div className="container mx-auto flex h-14 items-center gap-4 px-4 sm:px-6">
        <Link href="/" className="flex items-center shrink-0">
          <Image src="/logo.png" alt="FlowWave" width={120} height={40} className="h-7 sm:h-8 w-auto object-contain" priority />
        </Link>
        <nav className="flex items-center gap-0.5 flex-1">
          <a
            href="#features"
            className={cn(buttonVariants({ variant: "ghost", size: "sm" }), "text-muted-foreground hover:text-foreground text-xs sm:text-sm px-2 sm:px-3")}
          >
            Features
          </a>
        </nav>
...
function ProjectHubWindow() {
  ...
  return (
    <div className="rounded-xl border border-border bg-card shadow-2xl shadow-primary/10 overflow-hidden text-left">
      <div className="border-b border-border/70 bg-card/60 px-4 sm:px-5 py-3">
        ...
        <div className="flex gap-0.5 overflow-x-auto">
          {tabs.map((t, i) => (
            <span key={t} className={cn(
              "px-2.5 sm:px-3 py-1.5 text-[11px] font-medium whitespace-nowrap border-b-2",
  ...
  return (
    <div className="rounded-xl border border-border bg-card shadow-2xl shadow-primary/10 overflow-hidden text-left">
      <div className="flex h-[220px]">
        <div className="w-32 sm:w-36 shrink-0 border-r border-border bg-card/40 flex flex-col">
              ...
              <thead>
                <tr className="border-b border-border/60 bg-muted/30">
                  {["id", "name", "role", "created"].map((c) => (
                    <th key={c} className="text-left px-3 py-1.5 font-semibold text-muted-foreground whitespace-nowrap">{c}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-b border-border/30">
                    ...
                    <td className="px-3 py-1.5 text-foreground/80 whitespace-nowrap">{r.name}</td>
                    ...
                    <td className="px-3 py-1.5 text-muted-foreground font-mono whitespace-nowrap">{r.created}</td>
                  </tr>
                ))}
              </tbody>
            </table>
    ...
    <div className="flex flex-col min-h-screen overflow-x-hidden">
      ...
      <main className="flex-1">
        ...
        <section className="relative overflow-hidden hero-glow">
          <div className="container mx-auto px-4 sm:px-6 py-12 sm:py-16 md:py-24 lg:py-32 grid md:grid-cols-2 gap-8 md:gap-12 items-center">
            <div className="flex flex-col gap-5 md:gap-6">
              <Badge variant="secondary" className="gap-1.5 w-fit border border-primary/30 bg-primary/10 text-primary text-[11px] sm:text-xs">
                <Sparkles className="size-3 shrink-0" />
                <span className="hidden sm:inline">Project Hub · Discord · Supabase · Python</span>
                <span className="sm:hidden">Discord Bot Platform</span>
              </Badge>
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-1">
                <Link href="/projects" className={cn(buttonVariants({ size: "lg" }), "gap-2 shadow-lg shadow-primary/25 w-full sm:w-auto justify-center")}>
                  Jetzt starten
              ...
              <div className="grid grid-cols-2 gap-2 md:hidden pt-1">
                {[
                  { icon: Code2, label: "Code Editor", color: "text-blue-400" },
                  { icon: Database, label: "Supabase", color: "text-emerald-400" },
                  { icon: KeyRound, label: "Env Variables", color: "text-amber-400" },
                  { icon: Rocket, label: "1-Click Deploy", color: "text-primary" },
                ].map(({ icon: Icon, label, color }) => (
                  <div key={label} className="rounded-lg border border-border/50 bg-card/50 p-2.5 flex items-center gap-2">
        ...
        <section className="border-y border-border/60 bg-muted/20 py-10 sm:py-14 md:py-16">
          <div className="container mx-auto px-4 sm:px-6">
            <div className="text-center mb-7 sm:mb-10">
              <Badge variant="secondary" className="mb-3 border border-border">Neu</Badge>
              <h2 className="text-xl sm:text-2xl font-bold tracking-tight">Project Hub</h2>
        ...
        {/* Features */}
        <section id="features" className="container mx-auto px-4 sm:px-6 py-10 sm:py-14 md:py-16">
          <h2 className="text-xl sm:text-2xl font-bold text-center mb-3 tracking-tight">Alles was du brauchst</h2>
          <p className="text-muted-foreground text-sm text-center mb-8 md:mb-10 max-w-md mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
            {features.map((f) => (
              <div
                key={f.title}
                className="rounded-xl border border-border bg-card p-4 sm:p-5 flex flex-col gap-3 sm:gap-3.5 hover:border-primary/30 transition-colors"
              >
        ...
        <section className="container mx-auto px-4 sm:px-6 py-10 sm:py-14 md:py-16">
          <h2 className="text-xl sm:text-2xl font-bold text-center mb-8 sm:mb-12 tracking-tight">So einfach geht&apos;s</h2>
            ...
            {steps.map((s, i) => (
              <div key={s.n} className="flex gap-4">
                <div className="flex flex-col items-center shrink-0">
                  <div className="size-9 rounded-full bg-primary flex items-center justify-center text-primary-foreground font-bold text-sm shadow-md shadow-primary/30">
                    {s.n}
                  </div>
                  {i < steps.length - 1 && (
                ...
                <div className={cn("pt-1", i < steps.length - 1 ? "pb-6" : "pb-0")}>
          ...
          <div className="hidden md:block relative">
            <div className="absolute top-5 left-[calc(16.67%+1rem)] right-[calc(16.67%+1rem)] h-px bg-border" />
        ...
        <section className="border-t border-border/60 bg-muted/20 py-12 sm:py-16">
          <div className="container mx-auto px-4 sm:px-6 text-center space-y-4 sm:space-y-5">
            <h2 className="text-2xl sm:text-3xl font-bold tracking-tight">Bereit loszulegen?</h2>
            <p className="text-muted-foreground text-sm sm:text-base max-w-sm mx-auto">
              Kostenlos starten. Keine Kreditkarte, kein Setup.
            </p>
      ...
      <footer className="border-t border-border py-6 sm:py-7">
        <div className="container mx-auto px-4 sm:px-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-sm text-muted-foreground">
          ...
          <p className="text-xs sm:text-sm text-center">Gebaut für die Discord-Bot-Community.</p>
