...
export async function GET(_: NextRequest, { params }: { params: Promise<{ botId: string }> }) {
  ...
  const sb = createAdminClient();
  const { data } = await sb.from("project_folders").select("path").eq("bot_id", botId);
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ botId: string }> }) {
  ...
  const sb = createAdminClient();
  await sb.from("project_folders").delete().eq("bot_id", botId);
  if (paths.length > 0) {
    await sb.from("project_folders").insert(paths.map((p) => ({ bot_id: botId, path: p })));
  }
